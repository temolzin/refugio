...
@section('css')
    <link rel="stylesheet" href="{{ asset('css/roles/rol.css') }}">
@stop

@section('js')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('[data-card-widget="collapse"]').forEach(function(button) {
                button.addEventListener('click', function() {
                    var card = button.closest('.card');
                    var cardBody = card.querySelector('.card-body');
                    if (cardBody.style.display === 'none') {
                        cardBody.style.display = 'block';
                        button.querySelector('.fa').classList.replace('fa-plus', 'fa-minus');
                    } else {
                        cardBody.style.display = 'none';
                        button.querySelector('.fa').classList.replace('fa-minus', 'fa-plus');
                    }
                });
            });
        });
